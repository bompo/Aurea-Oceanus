...
public class WinScreen extends DefaultScreen implements InputProcessor {
	...
	public WinScreen(Game game, Boolean win) {
		super(game);
	...
	public void resize(int width, int height) {
		...
		if(win) {
			cam.position.set(0, 35.5f, 16f);
			cam.direction.set(0, -0.5f, -1);
		} else {
...
//						System.out.println("blocked");
	...
	public void render(float deltaTime) {
		...
		if(win) {
			if(startTime<20) {		
				cam.lookAt(submarine.position.x, submarine.position.y, submarine.position.z);
				if(startTime>2) {
					submarine.position.x += delta*10.f;
					submarine.position.y += delta *10.f;
					submarine.calculateMatrix();
				}
			}
		}
		...
		batch.begin();		
			font.setScale(5);
			if(!win) {
				font.drawMultiLine(batch, "Lost in Deep Sea\nGAME OVER",20, 240);
			} else {
				font.drawMultiLine(batch, "You Win!",20, 240);
			}
			font.setScale(1);		
		batch.end();
	...
	private void renderScene() {
		...
		if(win) {
			// render ship	
			if(cam.frustum.sphereInFrustum(submarine.collisionPosition,2000)) {
				modelShipTex.bind(0);
